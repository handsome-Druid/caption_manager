...
class AutoRemoveService:
    ...
    async def run(self, config: AutoRemoveConfig, folder: FolderPath):
        ...
        async with lock:
            ...
            blacklist_tags = await asyncio.to_thread(self.blacklist_tags.read)
            BlacklistService.run(captions, blacklist_tags)
            if config.overlap:
                overlap_tags_list = await asyncio.to_thread(self.overlap_tags.read)
                OverlapService.run(captions, overlap_tags_list)
            character_tags = await asyncio.to_thread(self.character_tags.read)
